...
//  FUNCIÓN PARA MOSTRAR/OCULTAR LA CANCHA 2D
...
// ==== FUNCIÓN PARA LOS NOMBRES ==== //
function seleccionarGoleador(equipo, seedObj, esGol = false) {
    // Filtro robusto que cruza ID numérico, String y Nombre para no errar de equipo jamás
    ...
    // Fallback seguro por si la base de datos no me da jugadores en la consulta
    if (jugadoresEquipo.length === 0) return "Jugador de " + equipo.nombre;

    let pool = jugadoresEquipo; 

    // Si es un gol, le damos prioridad a los datos de la base de datos (porcentaje, rol, medio, delantero)
    if (esGol) {
        ...
        if (usanPorcentaje) {
            ...
            // Si no hay porcentajes numéricos, filtramos por sus calificados textuales
            ...
            // 85% de probabilidad de que seleccione a uno de estos que filtró como ofensivos
...
// ==== PRECALCULAR FUTBOL CON MÁS DINAMISMO Y SAQUE DEL MEDIO ==== //
function precalcularFutbol(partido, seedObj) {
    ...
    // Goles LOCALES y saque del medio
    ...
    // Goles VISITANTES y saque del medio
    ...
    // DINAMISMO (de 20 a 40 eventos por partido)
    ...
    for(let m=1; m<=48; m++) {
        ...
        for(let p=0; p<posesionesL; p++) {
            if(seededRandom(seedObj.val++) < probL + 0.05) { 
                ...
                ptsL += totalPts;
                partido.eventos.push({ equipo: 'L', minuto: m, pts: totalPts, jugador: seleccionarGoleador(partido.local, seedObj, true), procesado: false }); // <--- Corregido
            }
        }
        ...
        for(let p=0; p<posesionesV; p++) {
            if(seededRandom(seedObj.val++) < probV + 0.05) {
                ...
                let totalPts = r < 0.20 ? 1 : (r < 0.65 ? 2 : 3);
                ptsV += totalPts;
                partido.eventos.push({ equipo: 'V', minuto: m, pts: totalPts, jugador: seleccionarGoleador(partido.visitante, seedObj, true), procesado: false }); // <--- Corregido
            }
        }
    }
...
function cicloDeportes() {
    ...
    if (faseApuestasAbierta) {
        ...
        timerDisplay.innerText = `${tiempoFase}s`;
        statusDisplay.innerText = `🏆 Competiciones en Vivo (Nuevo ciclo de apuestas en ${tiempoFase}s)`;
...
function renderizarPartidos() {
    ...
    partidosParaMostrar.forEach(partido => {
        ...
        matchCard.innerHTML = `
            ...
            <button class="btn-toggle-cancha" onclick="toggleCancha('${partido.id}')">📺 Ver Partido Animado</button>
                ...
                if (ev.minuto <= minutoReal && !ev.procesado) {
                    ...
                    if(deporte === 'Futbol') {
                        ...
                        if (ev.tipo === 'GOL') {
                            ...
                            partido.eventosUI.unshift(`⏱️ <b>${minText}</b> - ⚽ ${ev.texto}`);
                        } else {
                            partido.eventosUI.unshift(`⏱️ <b>${minText}</b> - 🏃 ${ev.texto}`);
                        }

                        // ACTUALIZAR POSICIÓN EN LA CANCHA 2D EN TIEMPO REAL
...
function renderizarMisPartidosEnVivo() {
    ...
    partidosEnVivo.forEach(p => {
        let resText = p.deporte === 'UFC' ? "🥊 EN PELEA" : `${p.golesL} : ${p.golesV}`;
...
//  PANELES SEGÚN  DISPOSITIVO 
function organizarPaneles() {
    const ancho = window.innerWidth;
    
    // Identifica contenedores originales
    const ticketFlotante = document.getElementById('bet-ticket');
    const panelHistorial = document.getElementById('bet-history');
    const panelEnVivo = document.querySelector('.live-bets-panel');
    const contenedorDerecho = document.querySelector('.side-panels');
    const contenedorIzquierdo = document.querySelector('.left-panels');
    
    const boletaItems = document.getElementById('ticket-items');
    const boletaSummary = document.querySelector('.ticket-summary');

    if (!ticketFlotante || !panelHistorial || !contenedorDerecho) return;

    if (ancho <= 768) {
        // ==========================================
        // MODO CELULAR: Agrupar todo dentro del ticket
        // ==========================================
        if(panelEnVivo) ticketFlotante.appendChild(panelEnVivo);
        ticketFlotante.appendChild(panelHistorial);
        
        // Ocultar cabeceras repetidas 
        if(panelEnVivo && panelEnVivo.querySelector('.ticket-header')) panelEnVivo.querySelector('.ticket-header').style.display = 'none';
        if(panelHistorial.querySelector('.ticket-header')) panelHistorial.querySelector('.ticket-header').style.display = 'none';

        // Si entran al celular y no hay pestaña activa, forzamos a mostrar la Boleta
        if (!document.querySelector('#tabs-celular button.active')) {
            cambiarPestanaMobile('boleta');
        }
        ...
        // MODO PC
        // ==========================================
        if(panelEnVivo) contenedorIzquierdo.appendChild(panelEnVivo);
        contenedorDerecho.appendChild(panelHistorial);
        
        // Mostrar sus cabeceras originales
        if(panelEnVivo && panelEnVivo.querySelector('.ticket-header')) panelEnVivo.querySelector('.ticket-header').style.display = 'flex';
        if(panelHistorial.querySelector('.ticket-header')) panelHistorial.querySelector('.ticket-header').style.display = 'flex';

        // todo visible PC 
        if(panelEnVivo) panelEnVivo.style.display = 'block';
        panelHistorial.style.display = 'block';
        
        if(boletaItems) boletaItems.style.display = 'block';
        if(boletaSummary) boletaSummary.style.display = 'block';
    }
}

// Ejecutar la función cuando carga la página y cuando cambian el tamaño de la ventana
document.addEventListener('DOMContentLoaded', organizarPaneles);
window.addEventListener('resize', organizarPaneles);


//  CAMBIAR PESTAÑAS (SOLO FUNCIONA EN CELULAR)
function cambiarPestanaMobile(seccion) {
    if (window.innerWidth > 768) return; // En PC bloqueamos esta función completamente

    const boletaItems = document.getElementById('ticket-items');
    const boletaSummary = document.querySelector('.ticket-summary');
    const panelEnVivo = document.querySelector('.live-bets-panel');
    const panelHistorial = document.getElementById('bet-history');
    const ticketHeaderTitle = document.querySelector('#bet-ticket .ticket-header h3');

    //  Quitar el color activo de todos los botones
    document.querySelectorAll('.tab-btn').forEach(btn => btn.classList.remove('active'));
    
    //  Ocultar los contenidos temporalmente
    if(boletaItems) boletaItems.style.display = 'none';
    if(boletaSummary) boletaSummary.style.display = 'none';
    if(panelEnVivo) panelEnVivo.style.display = 'none';
    if(panelHistorial) panelHistorial.style.display = 'none';

    //  Mostrar únicamente lo que el usuario tocó
    if (seccion === 'boleta') {
        document.getElementById('btn-tab-boleta').classList.add('active');
        if(boletaItems) boletaItems.style.display = 'block';
        if(boletaSummary) boletaSummary.style.display = 'block';
        if(ticketHeaderTitle) ticketHeaderTitle.innerText = "Boleta de Apuesta";
    } 
    else if (seccion === 'envivo') {
        document.getElementById('btn-tab-envivo').classList.add('active');
        if(panelEnVivo) panelEnVivo.style.display = 'block';
        if(ticketHeaderTitle) ticketHeaderTitle.innerText = "Partidos en Vivo";
    } 
    else if (seccion === 'historial') {
        document.getElementById('btn-tab-historial').classList.add('active');
        if(panelHistorial) panelHistorial.style.display = 'block';
        if(ticketHeaderTitle) ticketHeaderTitle.innerText = "Mis Apuestas";
    }
}