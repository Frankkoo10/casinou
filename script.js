...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Navegación de la barra superior (Botones estilo Betano)
    ...
    navItems.forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            // Agrega la clase "active" al botón que clickeaste
            e.target.classList.add('active');

            if (e.target.id === 'btn-nav-juegos') {
                // Muestra la sección de juegos y oculta la de "próximamente"
                juegosSection.classList.remove('hidden');
                if (soonSection) soonSection.classList.add('hidden');
            } else {
                // Si aprietas lo otro, te manda a "próximamente" ocultando juegos
                juegosSection.classList.add('hidden');
                if (soonSection) {
                    soonSection.classList.remove('hidden');
                    // Cambia el título dependiendo de qué botón tocaste
                    soonTitle.innerText = e.target.innerText + " (Próximamente)";
                }
            }
        });
    });
