...
// ==== AQUÍ ESTÁ LA NUEVA FUNCIÓN TOTALMENTE REPARADA PARA LOS NOMBRES ==== //
function seleccionarGoleador(equipo, seedObj, esGol = false) {
    ...
    // Fallback seguro por si la base de datos no arroja jugadores en la consulta
    if (jugadoresEquipo.length === 0) return "Jugador de " + equipo.nombre;

    let pool = jugadoresEquipo; 

    // Si es un evento de GOL, le damos prioridad a los datos de la base de datos (porcentaje, rol, medio, delantero)
    if (esGol) {
...
function precalcularFutbol(partido, seedObj) {
    ...
    // MUCHO MÁS DINAMISMO (de 20 a 40 eventos por partido)
...
// ==== RENDERIZADO CON LA NUEVA CANCHA 2D ESTILO BETANO ==== //
function renderizarPartidos() {
    ...
    partidosParaMostrar.forEach(partido => {
        ...
        // 1. Preparamos la cancha 2D SOLO si es Fútbol
        let canchaHTML = '';
        if (partido.deporte === 'Futbol') {
            canchaHTML = `
        ...
        matchCard.innerHTML = `
            ...
            <!-- ESTE BOTÓN Y LOS RELATOS AHORA APARECEN EN TODOS LOS DEPORTES -->
            <button class="btn-toggle-cancha" onclick="toggleCancha('${partido.id}')">📺 Ver Partido Animado / Relatos</button>
            <div id="cancha-wrapper-${partido.id}" class="hidden">
                
                ${canchaHTML} <!-- Se inyecta la cancha solo si es fútbol -->
...
// ==== ACTUALIZADOR DE CANCHA 2D EN TIEMPO REAL ==== //
function actualizarCancha2D(partidoId, xPercent, yPercent, texto, tipo, jugador, equipo, nombreLocal, nombreVisita) {
    ...
    if (tipo === 'GOL') {
        ...
        // POSESIÓN NORMAL: DORADO ELEGANTE
