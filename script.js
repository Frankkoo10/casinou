...
function precalcularBasketball(partido, seedObj) {
    ...
    for(let m=1; m<=48; m++) { // 48 segundos simulando 48 minutos
        
        // Equipo Local: Simulamos posesiones (entre 1 y 3 por minuto para ritmo de la NBA)
        let posesionesL = Math.floor(seededRandom(seedObj.val++) * 3) + 1; 
        for(let p=0; p<posesionesL; p++) {
            if(seededRandom(seedObj.val++) < probL + 0.05) { // Probabilidad de acierto ajustada
                let r = seededRandom(seedObj.val++);
                // Estadísticas NBA distribuidas para dar los puntos correspondientes
                // ~20% tiros libres (1pt), ~45% dobles/bandejas (2pts), ~35% triples (3pts)
        ...
        // Equipo Visitante: Simulamos posesiones
    ...
    // Si hay empate, desempate rápido
    if (ptsL === ptsV) ptsL += 2;
...
function precalcularUFC(partido, seedObj) {
    ...
    partido.golesFinalesL = winner === 'L' ? 1 : 0; // Representa victorias
...
function precalcularTenis(partido, seedObj) {
    ...
    let totalGames = 0;
    let serverL = seededRandom(seedObj.val++) < 0.5; // Comienza sirviendo
    let gameEventsTemp = [];
    ...
    while (setsL < 3 && setsV < 3) {
        ...
        while (true) {
            // El sacador tiene mayor probabilidad de ganar su game
            ...
            serverL = !serverL; // Cambia el servicio en el próximo game

            // Gana el set si llega a 6 games con 2 de diferencia, o si gana 7 en empate 6-6 (tiebreak simple)
    ...
    // Distribuimos equitativamente a lo largo del tiempo que dura tenis
...
function cicloDeportes() {
    ...
    // Actualizar la interfaz principal del temporizador
    ...
    // Limpiar boleto no confirmado al cerrarse las apuestas globales
    ...
    // Correr simulaciones exactas si estamos en tiempo de juego
    if (!faseApuestasAbierta) {
        let tiempoJugadoGlobal = secActuales - globalBetTime + 1; // De 1 hasta 95

        for (let deporte in configDeportes) {
            let conf = configDeportes[deporte];

            // Simula el progreso, permitiendo "ponerse al día" (catch-up) si el usuario entró tarde
            let minutoASimular = Math.min(tiempoJugadoGlobal, conf.playTime);
            simularMinutoPartidoPorDeporte(deporte, minutoASimular);

            // Si este deporte alcanzó o superó el final exacto de su propia duración, liquidamos y forzamos update
            ...
            if (tiempoJugadoGlobal >= conf.playTime && partidosDelDeporte.length > 0 && !partidosDelDeporte[0].finalizado) {
                ...
                simularMinutoPartidoPorDeporte(deporte, conf.playTime); // Forzar última actualización visual
            }
        }
    }
}
    ...
    partidosActuales.filter(p => p.deporte === deporte).forEach(partido => {
        ...
        if (!partido.finalizado) {
            partido.eventos.forEach(ev => {
                // IMPORTANTÍSIMO: ev.minuto <= minutoReal garantiza el "Catch-Up" o puesta al día 
                // cuando alguien recarga la página o entra tarde.
                if (ev.minuto <= minutoReal && !ev.procesado) {
                    ...
                    if(deporte === 'Futbol') {
                        ...
                        if(ev.final) {
                            ...
                            partido.finalizadoGlobal = true; // Activa la detención temprana (KO/SUB)
                        } else {
                            if (!partido.finalizadoGlobal) { // No mostrar eventos de golpes si ya finalizó por KO
                                let nEq = ev.equipo === 'L' ? partido.local.nombre : partido.visitante.nombre;
        ...
        if (deporte === 'Futbol') {
            ...
            if (minutoReal >= playLimit || partido.finalizadoGlobal) {
                displayMinuto = "FINAL";
                partidoTerminado = true;
                partido.finalizadoGlobal = true; // Sella el partido globalmente
            }
        }
        ...
        // Si recargan la página, huboEvento es true al ponerse al día, o si ya terminó, forzamos la vista
...
window.toggleSeleccion = function(partidoId, tipo, cuota, labelDesc, partidoNombres) {
    ...
    if (!p || !faseApuestasAbierta) return; // Bloqueo global estricto
...
window.confirmarApuesta = function() {
    ...
    if (!faseApuestasAbierta) return; // Rechaza contundentemente si se venció el global
    ...
    apuestasUsuario.forEach(apuesta => {
        ...
        for (let sel of apuesta.selecciones) {
            ...
            if (partidoReal) {
                ...
                partidoExtraviado = true; // El evento ya se limpió de la memoria o no existía
            }
        }
