...
// Configuración unificada (Timer Global de 90s para apuestas)
const globalBetTime = 90; 
const globalPlayTime = 95; // 95s para contemplar el tiempo extra en fútbol
const globalCycleLength = globalBetTime + globalPlayTime; // 185s en total

// Cada deporte finaliza y liquida apuestas en sus tiempos exactos:
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btn-confirm-deposit').addEventListener('click', () => autoRecarga('deposit-amount', 'deposit-msg'));
    
    // Botón de depósito para móvil
    document.getElementById('btn-confirm-deposit-mobile').addEventListener('click', () => {
        autoRecarga('deposit-amount-mobile', 'deposit-msg-mobile');
    });
    ...
    navItems.forEach(item => {
        item.addEventListener('click', (e) => {
            e.preventDefault(); 
            // Guardar en localStorage para recordar en qué pestaña estamos
            localStorage.setItem('casino_active_tab', e.target.id);
...
supabaseClient.auth.onAuthStateChange((event, session) => {
    if (session) {
        ...
        let activeTab = localStorage.getItem('casino_active_tab') || 'btn-nav-juegos';
        const tabToClick = document.getElementById(activeTab);
        if(tabToClick) {
            tabToClick.classList.add('active');
            if (activeTab === 'btn-nav-juegos') {
                juegosSection.classList.remove('hidden');
            } else if (activeTab === 'btn-nav-deportes') {
                deportesSection.classList.remove('hidden');
                inicializarDeportes();
            } else if (activeTab === 'btn-nav-ranking') {
                rankingSection.classList.remove('hidden');
                cargarRanking();
            }
        }
...
function autoRecarga(inputId, msgId) {
    ...
    if (now - lastDeposit < cooldown) {
        ...
        document.getElementById(msgId).innerText = `Espera ${timeLeft} min.`;
        document.getElementById(msgId).style.color = '#ff4d4d';
        return;
    }

    const amount = parseFloat(document.getElementById(inputId).value);
    if (isNaN(amount) || amount <= 0 || amount > 10000) {
        document.getElementById(msgId).innerText = 'Monto inválido (Máx $10k).';
        document.getElementById(msgId).style.color = '#ff4d4d';
        return;
    }
    ...
    document.getElementById(msgId).innerText = '¡Recarga exitosa!';
    document.getElementById(msgId).style.color = '#2ecc71';
...
async function cargarHistorialDesdeBD() {
    ...
    // Ejecutar evaluación de apuestas offline (partidos que terminaron mientras no estabas)
    sincronizarApuestasOffline();
}
...
async function inicializarDeportes() {
    ...
    if (!timerInterval) {
        // Inicializar sincronización basada en el tiempo actual exacto
        const now = Math.floor(Date.now() / 1000);
        globalRoundId = Math.floor(now / globalCycleLength);
...
window.cambiarDeporte = function(nuevoDeporte) {
    ...
    // Forzamos actualización inmediata de la UI
    cicloDeportes();
    renderizarPartidos();
}
...
function seleccionarGoleador(equipoId, seedObj) {
    let jugadoresEquipo = jugadoresGlobales.filter(j => j.equipo_id === equipoId);
    if (jugadoresEquipo.length === 0) return "Competidor/Jugador";
...
function precalcularFutbol(partido, seedObj) {
    ...
    partido.cuotas = {
        L: (1 / (probL * 0.75 * 1.15)).toFixed(2),
        E: (1 / (probE * 1.15)).toFixed(2),
        V: (1 / (probV * 0.75 * 1.15)).toFixed(2),
        LE: (1 / ((probL*0.75 + probE) * 1.15)).toFixed(2),
        VE: (1 / ((probV*0.75 + probE) * 1.15)).toFixed(2),
    ...
    for(let i=0; i<partido.golesFinalesL; i++) {
        partido.eventos.push({ equipo: 'L', minuto: Math.floor(seededRandom(seedObj.val++) * maxMinutos) + 1, jugador: seleccionarGoleador(partido.local.id, seedObj), procesado: false });
    }
    for(let i=0; i<partido.golesFinalesV; i++) {
        partido.eventos.push({ equipo: 'V', minuto: Math.floor(seededRandom(seedObj.val++) * maxMinutos) + 1, jugador: seleccionarGoleador(partido.visitante.id, seedObj), procesado: false });
    }
    ...
    for(let m=1; m<=48; m++) { 
        ...
        for(let p=0; p<posesionesL; p++) {
            if(seededRandom(seedObj.val++) < probL + 0.05) { 
                ...
                ptsL += totalPts;
                partido.eventos.push({ equipo: 'L', minuto: m, pts: totalPts, jugador: seleccionarGoleador(partido.local.id, seedObj), procesado: false });
            }
        }
        ...
        for(let p=0; p<posesionesV; p++) {
            if(seededRandom(seedObj.val++) < probV + 0.05) {
                ...
                let totalPts = r < 0.20 ? 1 : (r < 0.65 ? 2 : 3);
                ptsV += totalPts;
                partido.eventos.push({ equipo: 'V', minuto: m, pts: totalPts, jugador: seleccionarGoleador(partido.visitante.id, seedObj), procesado: false });
            }
        }
    }
...
function iniciarRondaSincronizadaPorDeporte(deporte, roundId) {
    ...
    // Limpiar partidos viejos de ESTE deporte
    ...
    for (let liga in equiposPorLiga) {
        ...
        for (let i = 0; i < mezclados.length - 1; i += 2) {
            ...
            let nuevoPartido = {
                id: `match_${local.id}_${visitante.id}_${deporte}`,
                deporte: deporte,
                liga: liga,
                local, visitante, 
...
function cicloDeportes() {
    ...
    // Iniciar nueva ronda global para todos
    ...
    if (faseApuestasAbierta) {
        ...
        timerDisplay.innerText = `${tiempoFase}s`;
        statusDisplay.innerText = `🏆 Competiciones en Vivo (Nuevo ciclo en ${tiempoFase}s)`;
            ...
            partido.eventos.forEach(ev => {
                if (ev.minuto <= minutoReal && !ev.procesado) {
                    ...
                    if(deporte === 'Futbol') {
                        ...
                        partido.eventosUI.unshift(`⏱️ <b>${minText}</b> - ⚽ ¡Gol de ${nEq}! (<i>${ev.jugador}</i>)`);
                    } 
                        ...
                        partido.eventosUI.unshift(`⏱️ <b>Q${Math.ceil(ev.minuto/12)} - ${ev.minuto}s</b> - 🏀 ${ptStr} de ${nEq} (<i>${ev.jugador}</i>)`);
                    }
                    else if (deporte === 'UFC') {
                        if(ev.final) {
                            partido.golesL = partido.resultado.ganador === 'L' ? 1 : 0;
                            partido.golesV = partido.resultado.ganador === 'V' ? 1 : 0;
                            partido.eventosUI.unshift(`⏱️ <b>FINAL</b> - 🥊 ${ev.tipo}`);
                            partido.finalizadoGlobal = true; 
                        } else {
                            if (!partido.finalizadoGlobal) { 
                                let nEq = ev.equipo === 'L' ? partido.local.nombre : partido.visitante.nombre;
                                partido.eventosUI.unshift(`⏱️ <b>${ev.minuto}s</b> - 💥 ${nEq} ${ev.tipo}`);
                            }
                        }
                    }
                        ...
                        if(ev.final) {
                            ...
                            partido.eventosUI.unshift(`⏱️ <b>FINAL</b> - 🎾 ${ev.text}`);
                            partido.finalizadoGlobal = true;
                        } else {
                            if (ev.isSetWin) {
                            ...
                            partido.eventosUI.unshift(`⏱️ <b>${ev.minuto}'</b> - 🎾 ${ev.text}`);
                        }
                    }
                }
            });
...
function actualizarTicketUI() {
    ...
    // Globito (Mobile)
    const bubble = document.getElementById('bet-bubble');
    const bubbleCount = document.getElementById('bubble-count');

    if (ticketSelecciones.length === 0) {
        ticketPanel.classList.add('hidden'); 
        bubble.classList.add('hidden');
        ticketPanel.classList.remove('show-mobile'); // Cerrar en móvil si está vacío
        return;
    }

    ticketPanel.classList.remove('hidden');
    bubble.classList.remove('hidden');
    bubbleCount.innerText = ticketSelecciones.length;
...
window.toggleMobileTicket = function() {
    ...
    ticket.classList.toggle('show-mobile');
}
...
window.confirmarApuesta = function() {
    ...
    // GUARDAR ID DE RONDA PARA RESOLUCIÓN OFFLINE
    let currentRoundIdForBet = globalRoundId;
    ticketSelecciones.forEach(s => s.roundId = currentRoundIdForBet);
    ...
    // Si estamos en celular, cerramos el ticket tras apostar
...
function renderizarHistorial() {
    ...
    if (!apuestasUsuario || apuestasUsuario.length === 0) {
        contenedor.innerHTML = '<p class="empty-msg">No hay apuestas recientes.</p>'; return;
    }
    contenedor.innerHTML = '';
...
// =========================================================================
// SISTEMA DE RESOLUCIÓN DE APUESTAS OFFLINE / AUSENTES
// =========================================================================
async function sincronizarApuestasOffline() {
    // Si no tenemos los equipos, los pedimos a la BD para poder simular los partidos pasados
    if (equiposGlobales.length === 0) {
        try {
    ...
    if (equiposGlobales.length === 0) return;

    // Calcular el round global actual
    const now = Math.floor(Date.now() / 1000);
    const currGlobalRound = Math.floor(now / globalCycleLength);
    ...
    for (let apuesta of apuestasUsuario) {
        if (apuesta.estado === 'pendiente' && !apuesta.resuelta) {
            ...
            for (let sel of apuesta.selecciones) {
                // Si la apuesta no tiene roundId guardado, asumimos que es del round anterior
                let pastRoundId = sel.roundId !== undefined ? sel.roundId : currGlobalRound - 1;

                if (pastRoundId >= currGlobalRound) {
                    todosResueltos = false; // Todavía se está jugando o no ha empezado
                    break;
                }

                // Recreamos exactamente el partido que ya terminó gracias a la semilla (seed) determinista
                let parts = sel.partidoId.split('_'); 
                let deporte = parts[3] || 'Futbol';
                let localId = parts[1];
                let visitanteId = parts[2];

                let seedObj = { val: (pastRoundId * 1000) + deporte.length };
                ...
                let partidoSimulado = null;
                for (let liga in equiposPorLiga) {
                    let mezclados = mezclarArrayDeterminista(equiposPorLiga[liga], seedObj.val++);
                    for (let i = 0; i < mezclados.length - 1; i += 2) {
                        let l = mezclados[i];
                        let v = mezclados[i+1];
                        let pId = `match_${l.id}_${v.id}_${deporte}`;
                        
                        let dummyMatch = { id: pId, deporte: deporte, local: l, visitante: v, eventos: [] };
                        if (deporte === 'Futbol') precalcularFutbol(dummyMatch, seedObj);
                        else if (deporte === 'Basketball') precalcularBasketball(dummyMatch, seedObj);
                        else if (deporte === 'UFC') precalcularUFC(dummyMatch, seedObj);
                        else if (deporte === 'Tenis') precalcularTenis(dummyMatch, seedObj);

                        if (pId === sel.partidoId) {
                            partidoSimulado = dummyMatch;
                        }
                    }
                }

                if (partidoSimulado) {
                    if (!verificarSeleccion(sel, partidoSimulado)) { boletaGanadora = false; }
                } else {
                    partidoExtraviado = true; // Si los equipos fueron borrados de la DB
                }
            }
            ...
            if (todosResueltos) {
                ...
                if (partidoExtraviado) {
                    apuesta.estado = 'cancelada';
                    apuesta.gananciaPosible = apuesta.monto;
                    gananciasTotales += apuesta.monto;
                } else if (boletaGanadora) {
                    apuesta.estado = 'ganada';
                    gananciasTotales += apuesta.gananciaPosible;
                } else {
                    apuesta.estado = 'perdida';
                }
                actualizarApuestaBD(apuesta.id, apuesta.estado, apuesta.resuelta);
                actualizadas = true;
            }
        }
    }
    ...
    if (actualizadas) {
        if (gananciasTotales > 0) {
            actualizarSaldoBD(gananciasTotales);
        }
        renderizarHistorial();
    }
}